...
## 其它后台语言
* java：稳定，安全，相对PHP学习成本稍高（也可以实现单线程）
* .net （vb做 .net , c#）
* python: 做人工智能
* C++ 只用C的很少(10年精通)
## Nodejs方向
...
* MVC（M：业务逻辑层，V：C：）
1. 大量AJAX请求的应用
1. javascript在nosql的应用
1. javascript运行从前台 到后台
## 特点
* 单线程
* 高并发
## 缺点：
1. 不 适合CPU密集性应用
1. 只支持单核CPU
1. 可靠性 低，一旦代码某个环节崩溃，整个系统都崩溃
1. 向下不兼容（少数）winner基于NODEJS开发的工具，开源组件库（所有的库统称NPM库）
